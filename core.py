...
class Lifegame():
    def __init__(self,n):
        self.field = [[1 for i in range(n)] for j in range(n)]
    def forward(self):
        self.nextField = [[1 for i in range(len(self.field))] for j in range(len(self.field))]
        ...
        for a in range(-1,2):
            for b in range(-1,2):
                if (i + a > 0 and j + b > 0 and i + a < len(self.field) and j + b < len(self.field)): 
                    if (self.field[i+a][j+b] == 0 and ((a != 0) or (b != 0))):
                        count += 1
        return count
